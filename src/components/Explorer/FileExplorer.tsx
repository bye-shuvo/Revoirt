...
const FileExplorer = () => {
  ...
  const setRemoteUserCount = useRemoteUserCount(
  ...
  const createFileEvent = (e: any): void => {
    if (
      !Array.from(inputFile.current?.children || []).includes(
        e.target as Element,
      )
    ) {
  ...
  const deleteFileEvent = (e: any): void => {
    if (
      !Array.from(contextMenuRef.current?.children || []).includes(
        e.target as Element,
      )
    ) {
      setDeletedPath("");
  ...
  const renameFileEvent = async (e: any): Promise<void> => {
    if (
      !Array.from(renameFile.current?.children || []).includes(
        e.target as Element,
      )
    ) {
  ...
  const renameEventHandler = async (fileName: string, filePath: string) => {
    ...
    changableFileRef.current = freashFiles?.find(
    ...
    <aside className="h-full bg-[#181818]">
      {showToast?.doShow && (
        <Toast
          ...
          duration={1500}
          onDone={() => {
            setShowToast({ doShow: false, type: "", message: "" });
          }}
          top="95%"
          left="50%"
        />
      ...
      <ul className="flex flex-col justify-center w-full">
          ...
          return (
            <li
              ...
              onContextMenu={(e) => {
                e.preventDefault();
                e.stopPropagation();
                setDeletedPath(file?.path);
                setIsContextMenuOpen(true);
              }}
              className={`flex gap-2 text-[1rem] cursor-pointer hover:bg-gray-500/50 p-2 relative text-white ${file?.path === path ? "bg-[#222222]" : ""}`}
            >
              <svg
                className="h-6 w-6"
                xmlns="http://www.w3.org/2000/svg"
              ...
              {file.name}
              {isContextMenuOpen && deletedPath === file?.path && (
                <div
                  ref={contextMenuRef}
                  className="flex flex-col border border-slate-600 bg-[#181818] absolute z-10 w-7/8 p-2 rounded-sm"
                >
                  <button
                    className="p-1 hover:bg-gray-700/50 rounded-sm"
                    onMouseDown={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      renameEventHandler(file?.name, file?.path);
                    }}
                  >
                    Rename...
                  </button>
        ...
        {isAddingNewFile && (
          <li
            ref={inputFile}
            className="flex gap-2 text-[1rem] cursor-text p-2 items-center"
          >
            <svg
              className="h-6 w-6"
              xmlns="http://www.w3.org/2000/svg"
            ...
            <input
              ...
              autoFocus={true}
              className="w-fit text-[1rem] border rounded-sm outline-none border-blue-400 text-white placeholder:text-white py-0.5"
              type="text"
            />
          </li>
        )}
