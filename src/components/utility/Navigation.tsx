...
const Navigation = () => {
  ...
  return (
    <nav className="bg-mist-900 h-[7%] w-full flex items-center">
      <ul className="w-full flex justify-between items-center px-3">
        ...
        <li id="right-utils" className="flex items-center gap-10">
          <div id="joined-users" className="flex relative justify-center items-center w-fit h-full">
            {
              Array.from({ length: 5 }).map((_, index) => {
                return <span key={index} className={`relative h-8 w-8 flex rounded-full bg-purple-500 border border-green-400`} style={{ left: index * -10 }}> <img src="" alt={`user-${index+1}`} /> </span>
              })
            }
          </div>
          <button id="collaborate" className="bg-sky-500 border-b-3 border-sky-700 cursor-pointer p-2 active:border-0" onClick={() => setIsCollaborating(true)}>Collaborate</button>
