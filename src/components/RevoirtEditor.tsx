    ...
    <>
      {
        (!path) ? <div className="h-full bg-inherit flex justify-center items-center flex-col gap-5 text-white font-jetbrains-mono no-scrollbar overflow-hidden">
          <h1 className="text-lg">
            <p className='text-xl text-purple-400'>Welcome</p>
            <p className='text-xl text-purple-400'>To</p>
            <p className='font-saira-stencil-one font-extrabold text-[11rem] leading-38 text-transparent bg-clip-text bg-linear-[140deg] from-purple-500 to-amber-400'>Revoirt</p>
          ...
          <div id='command-pallete' className='rounded-lg border-green-400 border-2 p-2'>{shortcuts.map((shortcut, index) => {
            return <pre key={index}>{shortcut.name} <svg className='h-5 w-5 inline' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path fill="rgb(255, 255, 255)" d="M598.6 342.6C611.1 330.1 611.1 309.8 598.6 297.3L470.6 169.3C458.1 156.8 437.8 156.8 425.3 169.3C412.8 181.8 412.8 202.1 425.3 214.6L498.7 288L64 288C46.3 288 32 302.3 32 320C32 337.7 46.3 352 64 352L498.7 352L425.3 425.4C412.8 437.9 412.8 458.2 425.3 470.7C437.8 483.2 458.1 483.2 470.6 470.7L598.6 342.7z" /></svg> {shortcut.command}</pre>
          })}</div>
        </div> :
